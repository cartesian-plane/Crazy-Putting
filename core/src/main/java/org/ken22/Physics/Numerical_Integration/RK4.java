...
//      System.out.println("k1: " + k1.toString());
        ...
        ArrayList<Double> k2 = compute_k_i(initial, stateVectors, timeStep, 0.5, funcx, funcy, k1, terrain, differentiator);
//      System.out.println("k2: " + k2.toString());

        ArrayList<Double> k3 = compute_k_i(initial, stateVectors, timeStep, 0.5, funcx, funcy, k2, terrain, differentiator);
//      System.out.println("k3: " + k3.toString());

        ArrayList<Double> k4 = compute_k_i(initial, stateVectors, timeStep, 1.0, funcx, funcy, k3, terrain, differentiator);
...
//      System.out.println("Final state vector: " + stateVector.getVector().toString());
    ...
    public ArrayList<Double> compute_k_i(ArrayList<Double> vector, ArrayList<GVec4> stateVectors, double timeStep, double subStep, IFunc<Double, Double> funcx, IFunc<Double, Double> funcy, ArrayList<Double> k_previous, Expression terrain, NumDerivationMethod differentiator) {

        ArrayList<Double> k_i = new ArrayList<Double>();
        ArrayList<Double> interim = interim_vector(vector, stateVectors, timeStep, subStep, funcx, funcy, k_previous, terrain, differentiator);
...
//      System.out.println("k_i: " + k_i.toString());
    ...
    public ArrayList<Double> interim_vector(ArrayList<Double> vector, ArrayList<GVec4> stateVectors, double timeStep, double subStep, IFunc<Double, Double> funcx, IFunc<Double, Double> funcy, ArrayList<Double> k_i, Expression terrain, NumDerivationMethod differentiator) {
